...
set destination=DMM.com

rem DMM.com LLC

REM domain/32
REM display.dmm.co.jp
REM point.dmm.com
REM www.dmm.com
REM bitcoin.dmm.com
set num=domain_32
...
rem 24
rem set ip_c_=223.252.113.0
rem set ip_c_=223.252.114.0
...
setlocal EnableDelayedExpansion
for /l %%i in (1,1,%count_ip_22%) do (
>nul 2>&1 route delete !ip_22_%%i!
>nul 2>&1 route add !ip_22_%%i! mask %mask_22% %gateway% if %if% metric 1
)
endlocal

setlocal EnableDelayedExpansion
for /l %%i in (1,1,%count_ip_20%) do (
>nul 2>&1 route delete !ip_20_%%i!
>nul 2>&1 route add !ip_20_%%i! mask %mask_20% %gateway% if %if% metric 1
)
endlocal
